<template>
  <LogsContainer>
    ...
    <div v-else-if="error" class="text-center py-6 sm:py-8">
      ...
      <p class="text-sm sm:text-base text-destructive">Failed to load failure logs</p>
    ...
    <div v-else-if="currentLogs.length === 0" class="text-center py-6 sm:py-8">
      <AppIcon icon="lucide:check-circle" size="40" class="sm:w-12 sm:h-12 mx-auto text-success mb-3 sm:mb-4" />
      <p class="text-sm sm:text-base text-muted-foreground">No failure logs found</p>
    </div>
...
const { fetchFailureLogs } = useLogs()

const currentPage = ref(1)
const pageSize = 50

const { data: logsData, pending, error, refresh: refreshLogs } = await useLazyAsyncData('logs-failures', async () => {
  return await fetchFailureLogs(currentPage.value, pageSize)
}, {
  server: false,
  watch: [currentPage]
})
