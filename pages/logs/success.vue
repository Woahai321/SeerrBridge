<template>
  <LogsContainer>
    ...
    <div v-else-if="error" class="text-center py-6 sm:py-8">
      ...
      <p class="text-sm sm:text-base text-destructive">Failed to load success logs</p>
    ...
    <div v-else-if="currentLogs.length === 0" class="text-center py-6 sm:py-8">
      ...
      <p class="text-sm sm:text-base text-muted-foreground">No success logs found</p>
    </div>
...
const { fetchSuccessLogs } = useLogs()

const currentPage = ref(1)
const pageSize = 50

const { data: logsData, pending, error, refresh: refreshLogs } = await useLazyAsyncData('logs-success', async () => {
  return await fetchSuccessLogs(currentPage.value, pageSize)
}, {
  server: false,
  watch: [currentPage]
})
