  ...
  <LogsContainer>
    ...
    <div v-if="pending" class="space-y-3 sm:space-y-4">
      <div v-for="i in 10" :key="i" class="animate-pulse">
        <div class="h-3 sm:h-4 bg-muted rounded w-3/4 mb-2 dark:bg-primary/20 dark:backdrop-blur-sm" />
        <div class="h-2.5 sm:h-3 bg-muted rounded w-1/2 dark:bg-primary/15 dark:backdrop-blur-sm" />
      </div>
    </div>
    ...
    <div v-else class="space-y-2 sm:space-y-4">
      <div 
        ...
        :key="log.id"
        class="flex items-start space-x-2 sm:space-x-3 p-3 sm:p-4 rounded-lg hover:bg-muted/50 transition-all duration-200 dark:bg-muted/10 dark:hover:bg-muted/30 dark:backdrop-blur-sm dark:border dark:border-primary/10"
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-1.5 sm:mt-2">
            ...
            <span v-if="log.type" class="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 bg-muted rounded-full whitespace-nowrap dark:bg-primary/20 dark:text-primary dark:border dark:border-primary/30">
              {{ log.type }}
            </span>
          </div>
        </div>
...
const { data: logsData, pending, error, refresh: refreshLogs } = await useLazyAsyncData('logs-all', async () => {
